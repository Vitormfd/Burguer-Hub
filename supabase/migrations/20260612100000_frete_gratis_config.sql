...
COMMENT ON COLUMN public.configuracoes.frete_gratis_ativo IS
  'Quando ativo, todos os pedidos delivery têm taxa de entrega zerada.';
COMMENT ON COLUMN public.configuracoes.frete_gratis_minimo IS
  'Valor mínimo do subtotal para frete grátis. NULL ou 0 desativa esta regra.';
...
CREATE OR REPLACE FUNCTION public.resolver_taxa_bairro(
  p_owner_id uuid,
  p_bairro text,
  p_taxa_informada numeric DEFAULT 0
)
RETURNS numeric
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  IF FOUND THEN
    RETURN GREATEST(COALESCE(v_taxa, 0), 0);
  END IF;

  RETURN GREATEST(COALESCE(p_taxa_informada, 0), 0);
END;
$$;
...
GRANT EXECUTE ON FUNCTION public.calcular_taxa_entrega_efetiva(uuid, public.tipo_entrega, numeric, numeric, boolean) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.resolver_taxa_bairro(uuid, text, numeric) TO anon, authenticated;
