ALTER TABLE public.categorias
  ADD COLUMN IF NOT EXISTS exclui_frete_gratis boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.categorias.exclui_frete_gratis IS
  'Produtos desta categoria impedem frete grátis automático (global, bairro ou valor mínimo). Cupom de frete grátis continua valendo.';

CREATE OR REPLACE FUNCTION public.carrinho_bloqueia_frete_gratis(
  p_owner_id uuid,
  p_produto_ids uuid[]
)
RETURNS boolean
...
AS $$
  SELECT EXISTS (
    ...
    WHERE p.owner_id = p_owner_id
      ...
      AND COALESCE(c.exclui_frete_gratis, false) = true
  );
$$;

CREATE OR REPLACE FUNCTION public.calcular_taxa_entrega_efetiva(
  p_owner_id uuid,
  p_tipo_entrega public.tipo_entrega,
  p_taxa_bairro numeric,
  p_subtotal numeric,
  p_cupom_zera_frete boolean DEFAULT false,
...
DECLARE
  ...
  v_bt public.bairros_taxas;
  v_taxa numeric := GREATEST(COALESCE(p_taxa_bairro, 0), 0);
BEGIN
  IF p_tipo_entrega = 'retirada' OR COALESCE(p_cupom_zera_frete, false) THEN
    RETURN 0;
  END IF;

  IF public.carrinho_bloqueia_frete_gratis(p_owner_id, p_produto_ids) THEN
    RETURN v_taxa;
  END IF;
  ...
  IF COALESCE(v_cfg.frete_gratis_ativo, false) THEN
    RETURN 0;
  END IF;

  SELECT * INTO v_bt
  FROM public.bairros_taxas bt
  WHERE bt.owner_id = p_owner_id
    AND bt.ativo = true
    AND lower(trim(bt.nome)) = lower(trim(COALESCE(p_bairro, '')))
  LIMIT 1;

  IF FOUND AND COALESCE(v_bt.frete_gratis_ativo, false) THEN
    RETURN 0;
  END IF;

  IF COALESCE(v_cfg.frete_gratis_minimo, 0) > 0
     AND COALESCE(p_subtotal, 0) >= v_cfg.frete_gratis_minimo THEN
    RETURN 0;
  END IF;

  IF FOUND
     AND COALESCE(v_bt.frete_gratis_minimo, 0) > 0
     AND COALESCE(p_subtotal, 0) >= v_bt.frete_gratis_minimo THEN
    RETURN 0;
  END IF;
...
BEGIN
  ...
  v_taxa_efetiva := public.calcular_taxa_entrega_efetiva(
    ...
    COALESCE(v_cupom_zera_frete, false),
    p_bairro,
    v_produto_ids
  );
...
GRANT EXECUTE ON FUNCTION public.carrinho_bloqueia_frete_gratis(uuid, uuid[]) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.calcular_taxa_entrega_efetiva(uuid, public.tipo_entrega, numeric, numeric, boolean, text, uuid[]) TO anon, authenticated;
